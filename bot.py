"""
ربات لاتاری TRON - نسخه نهایی حرفه‌ای با سیستم قرعه‌کشی - رفع خطا کامل
TRON Lottery Bot - Final Professional Version with Lottery System - Full Bug Fix
"""
...
class TronLotteryBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.args and len(context.args) > 0:
            ...
            logging.info(f"User {user.id} came through referral code: {referral_code} from user: {referred_by}")
        ...
        if not existing_user:
            ...
            logging.info(f"New user created: {user.id} with referral: {referred_by}")
            ...
            logging.info(f"Referral added for existing user: {user.id} from: {referred_by}")
    ...
    async def announce_winner(self, winner: Dict, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            logging.info(f"Winner announced to user {winner['user_id']} with prize ${winner['prize_amount']}")
            
        except Exception as e:
            logging.error(f"Failed to announce winner to {winner['user_id']}: {e}")
    ...
    def run(self):
        ...
        # تنظیم کدبندی پیشرفته برای حل مشکل encoding
        import sys
        import io
...
if __name__ == "__main__":
    ...
    if Config.BOT_TOKEN == "توکن_ربات_خودت_را_اینجا_قرار_ده":
        ...
        print("🔗 **رفرال:** سیستم رفرال کاملاً اصلاح شده")
        print("💎 **کدبندی:** مشکل encoding کاملاً حل شده")
    else:
        bot = TronLotteryBot()
        bot.run()
