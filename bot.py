"""
ربات لاتاری TRON - نسخه نهایی
TRON Lottery Bot - Final Version
"""
...
class TronLotteryBot:
    ...
    async def handle_lottery(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # ابتدا آدرس کیف پول را جداگانه بفرست
        wallet_message = (
            f"{self.lang.get_text(language, 'wallet_address_message')}\n\n"
    ...
    async def handle_users_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # همچنین نمایش خلاصه در متن
        users_text = f"👥 خلاصه کاربران (تعداد: {len(users)})\n\n"
        users_text += f"📈 کاربران فعال امروز: {len([u for u in users if u['last_active'] and u['last_active'].startswith(datetime.now().strftime('%Y-%m-%d'))])}\n"
        users_text += f"💰 مجموع موجودی: ${sum(u['balance_usd'] for u in users):.2f}\n"
        users_text += f"🎫 مجموع بلیط‌ها: {sum(u['ticket_count'] for u in users)}"
