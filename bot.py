"""
ربات لاتاری TRON - نسخه حرفه‌ای
TRON Lottery Bot - Professional Version
"""
    ...
    def get_text(language: str, key: str) -> str:
        ...
        texts = {
            'fa': {
                ...
                'copy_address': "📋 برای کپی کردن آدرس، روی آن کلیک کنید:",
...
class DatabaseManager:
    ...
    def get_all_users(self) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute('''
            SELECT u.user_id, u.username, u.first_name, u.balance_usd, u.created_at,
        ...
        return [{
            ...
            'balance_usd': u[3], 'created_at': u[4], 'ticket_count': u[5]
        } for u in users]
    ...
    def export_users_to_csv(self):
        ...
        with open(filename, 'w', newline='', encoding='utf-8') as file:
            ...
            writer.writerow(['User ID', 'Username', 'First Name', 'Balance USD', 'Tickets', 'Join Date'])
...
class PaymentProcessor:
    ...
    def process_transaction_hash(self, user_id: int, transaction_hash: str, language: str) -> Tuple[bool, str]:
        ...
        # تأیید خودکار پرداخت
        self.db.approve_payment(self._get_payment_id(transaction_hash))
...
class TronLotteryBot:
    ...
    async def show_main_menu(self, update: Update, language: str, user=None):
        ...
        if self.is_admin(user.id):
            keyboard.append([KeyboardButton(self.lang.get_text(language, 'admin_panel'))])
    ...
    async def handle_users_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # نمایش 10 کاربر اول
        users_text = f"👥 لیست کاربران (تعداد: {len(users)})\n\n"
        for i, user_data in enumerate(users[:10], 1):
            users_text += (
                f"{i}. {self.lang.get_text(language, 'user_id').format(id=user_data['user_id'])}\n"
                f"   {self.lang.get_text(language, 'username').format(username=user_data['username'] or 'N/A')}\n"
                f"   {self.lang.get_text(language, 'balance').format(balance=user_data['balance_usd'])}\n"
                f"   {self.lang.get_text(language, 'tickets').format(count=user_data['ticket_count'])}\n"
                f"   {self.lang.get_text(language, 'join_date').format(date=user_data['created_at'][:10])}\n\n"
            )
    ...
    async def handle_private_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        elif 'awaiting_private_message' in context.user_data:
    ...
    async def handle_transaction(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        transaction_hash = transaction_hash.lower()  # تبدیل به حروف کوچک
...
if __name__ == "__main__":
    ...
    if Config.BOT_TOKEN == "توکن_ربات_خودت_را_اینجا_قرار_ده":
        ...
        print("\n🔧 همچنین آیدی ادمین را در خط 19 تنظیم کنید / Also set admin ID in line 19")
    else:
        bot = TronLotteryBot()
        bot.run()
